...
bool NDArrayConverter::toMat(PyObject* o, Mat& m) {
    ...
    for (int i = ndims - 1; i >= 0 && !needcopy; i--) {
        ...
        //  2-dimensional cases b) transposed arrays, where _strides[] elements
        //  go in non-descending order c) flipped arrays, where some of
        //  _strides[] elements are negative
    ...
    if (needcopy) {
        // if (info.outputarg)
        // {
        //     failmsg("Layout of the output array %s is incompatible with
        //     cv::Mat (step[ndims-1] != elemsize or step[1] !=
